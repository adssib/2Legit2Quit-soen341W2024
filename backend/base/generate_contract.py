...
def generate_rental_agreement_pdf_and_send_email(file_path, user, reservation):
    ...
    f"Contact Number: {user.phone_number}",
    f"Email Address: {user.email}",
    f"Driver's License Number: {user.driver_license_number}",
    "",
    "2. Vehicle Information:",
    f"Make: {reservation.product.make}",
    f"Model: {reservation.product.model}",
    f"Year: {reservation.product.year}",
    f"License Plate Number: {reservation.product.license_plate_number}",
    f"Vehicle Identification Number (VIN): {reservation.product.vin}",
    f"Color: {reservation.product.color}",
    "",
    "3. Rental Details:",
